...
API 근거: 쿠폰 목록 모달에서 쿠폰을 선택할 때마다 아래쪽 버튼에 총 할인 금액을 표시하기 위해서 해당 API를 생성했다. request body에는 선택된 쿠폰의 id와 장바구니에 담은 상품 id, 제주도 및 도서산간 지역 여부를 보낸다.
...
API 근거: 클라이언트에서 사용가능한 쿠폰 id와 상품정보를 보내서 최대할인 쿠폰을 계산한다.
쿠폰 id는 최대 2개까지만 받을 수 있다. -> 최대 적용 가능 쿠폰 수가 2개이기 때문.
